...
define(['../js/config.js'],function () {
    requirejs(['jquery','goodlist'],function ($,gl) {
...
//                    ...............点击加入购物车，右侧的购物车信息更新.....................
            ...
            function delgoods(){
                $('.btn-remove').on('click',function () {
                    ...
                    //当商品总数减到0时，清空shopcart-list并添加empty信息
                    if($('.ym-nBar-cart-num em').html()==0){
                        $('.shopcart-list').empty();
                        $('<div class="empty">您的购物车里没有任何商品，赶快去挑选商品吧！</div>').appendTo($('.shopcart-list'))
                    }
                })
                ...
                function delCookies(){
                    $.each(carList,function(idx,item){
                        if(item){
                            if((item.guid)==$self.closest('li').attr('data-guid')){
